...
globalThis.handleGlobalFilterChange = () => {
   ...
   data = processedData.filter((d) => {
      ...
      const meetsTotdFreqBarConstraint =
         !totdFreqBar?.selectedDomain ||
         totdFreqBar.selectedDomain.length === 0 ||
         totdFreqBar.selectedDomain.includes(`${d.totd}`);
      const meetsShapeFreqBarConstraint =
         !shapeFreqBar?.selectedDomain ||
         shapeFreqBar.selectedDomain.length === 0 ||
         shapeFreqBar.selectedDomain.includes(`${d.ufo_shape}`);
      const meetsSeasonFreqBarConstraint =
         !seasonFreqBar?.selectedDomain ||
         seasonFreqBar.selectedDomain.length === 0 ||
         seasonFreqBar.selectedDomain.includes(`${d.season}`);
